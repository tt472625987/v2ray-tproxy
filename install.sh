...
# 策略路由：fwmark -> table（IPv4/IPv6）
if [ -n "$MARK" ] && [ -n "$ROUTE_TABLE" ]; then
	# IPv4：局域网目的走 main，避免本机输出流量误入 TPROXY 表
	[ -n "$LOCAL_SUBNET" ] && {
	...
	# IPv6：按需添加对应策略（当 ENABLE_IPV6=1 且 ip6tables 可用）
	if [ "${ENABLE_IPV6:-0}" = "1" ]; then
		# 局域网目的走 main（IPv6）
		[ -n "$LOCAL_SUBNET6" ] && {
			ip -6 rule del to "$LOCAL_SUBNET6" lookup main 2>/dev/null
			ip -6 rule add pref 99 to "$LOCAL_SUBNET6" lookup main 2>/dev/null
		}
		ip -6 rule del fwmark "$MARK" table "$ROUTE_TABLE" 2>/dev/null
		ip -6 route flush table "$ROUTE_TABLE" 2>/dev/null
		ip -6 rule add pref 100 fwmark "$MARK" lookup "$ROUTE_TABLE" 2>/dev/null
		ip -6 route add local ::/0 dev lo table "$ROUTE_TABLE" 2>/dev/null
	fi
fi
